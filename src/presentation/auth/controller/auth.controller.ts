...
export class AuthController {
  ...
  public loginUser = async (req: Request, res: Response) => {
    ...
    if (!loginUserDto)
      return res.status(400).json({ error: "El usuario no pudo ser creado" });
    ...
    new LoginUseCaseImpl(this.authRepository)
      ...
      .then(async (user) => {
        ...
        if (!accessToken)
          return res
            .status(500)
            .json({ error: "Error al generar access token" });
        ...
        if (!refreshToken)
          return res
            .status(500)
            .json({ error: "Error al generar refresh token" });
  ...
  public registerUser = async (req: Request, res: Response) => {
    ...
    new CreateUserUseCaseImpl(this.userRepository, this.bcrypt)
      ...
      .then(async (user) => {
        ...
        return res.status(201).json({
          ...
          message:
            "Usuario creado exitosamente. Revisa la bandeja de entrada de tu correo electrónico para verificar la cuenta.",
        });
      })
      .catch((error) => {
        return this.handleHttpStatusError(error, res);
  ...
  public retryVerifyEmail = async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json({ message: "Email de verificación enviado nuevamente" });
  };
  ...
  public resetPasswordSubmit = async (req: Request, res: Response) => {
    ...
    const [error, resetPasswordDto] = ResetPasswordDto.create({
      token,
      newPassword,
    });
