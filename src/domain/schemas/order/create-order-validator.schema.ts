...
/**
 * Schema para un item de la orden
 */
export const orderItemSchema = zod.object({
  ...
  batchId: zod
    .uuid({ error: "ID de batch inválido" })
    .optional(),
});
...
export const createOrderSchema = zod.object({
  ...
  items: zod
    .array(orderItemSchema, { error: "Items de la orden son requeridos" })
    .min(1, { error: "Debe haber al menos un producto en la orden" }),
  ...
  if (data.amountPaid < total) {
    ctx.addIssue({
      code: "custom",
      path: ["amountPaid"],
      message: `El monto pagado (${data.amountPaid} Bs) debe ser mayor o igual al total (${total.toFixed(2)} Bs)`,
    });
  }
});

export type CreateOrderSchema = zod.infer<typeof createOrderSchema>;
export type OrderItemSchema = zod.infer<typeof orderItemSchema>;
